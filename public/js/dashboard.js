...
const SELECTORS = {
    ...
    DEACTIVATE_ACCOUNT_BTN: '#deactivateAccountBtn',
...
function setupCommonEventListeners() {
    ...
    const deactivateAccountBtn = document.querySelector(SELECTORS.DEACTIVATE_ACCOUNT_BTN);
    if (deactivateAccountBtn) {
        deactivateAccountBtn.addEventListener('click', () => {
            const modal = document.querySelector(SELECTORS.DEACTIVATE_ACCOUNT_MODAL);
            if (modal) modal.style.display = 'flex';
        });
    }

    const deactivateAccountForm = document.querySelector(SELECTORS.DEACTIVATE_ACCOUNT_FORM);
    if (deactivateAccountForm) {
        deactivateAccountForm.addEventListener('submit', handleDeactivateAccountSubmit);
    }

    // Agregar manejador para el botón cancelar
    const cancelDeactivateBtn = document.querySelector('#deactivateAccountModal .cancel-btn');
...
function handleOrderActions(event) {
    const target = event.target;
    // Mantenemos la lógica anterior de los filtros
    const filterButton = target.closest('.filter-btn');
    if (filterButton) {
        return; // La otra función manejará esto
    }

    // NUEVA LÓGICA para el botón de detalles
    const detailsBtn = target.closest('.view-order-details-btn');
    if (detailsBtn) {
        const orderId = detailsBtn.dataset.orderId;
        displayOrderDetailsInModal(orderId);
    }
}
...
function renderOrderCard(order) {
    ...
    try {
        ...
        if (!isNaN(date.getTime())) {
            orderDateFormatted = date.toLocaleDateString('es-ES', { day: '2-digit', month: 'long', year: 'numeric' });
        }
    ...
    // La nueva estructura de la tarjeta, más simplificada
    return `
        <div class="order-card-new" data-order-id="${order._id}" data-status="${order.status?.toLowerCase() || 'pending'}">
            <div class="card-header">
                <span class="order-id">Pedido #${order.orderId || order._id}</span>
                <span class="status-badge-new status-${order.status?.toLowerCase() || 'pending'}">
            ...
            <div class="card-footer">
                <button class="details-btn view-order-details-btn" data-order-id="${order._id}">
                    Ver Detalles
                </button>
            </div>
        </div>
...
/**
 * Busca un pedido por su ID y muestra sus detalles en el modal.
 * @param {string} orderId - El ID del pedido a mostrar.
 */
function displayOrderDetailsInModal(orderId) {
    const order = dashboardState.userOrders.find(o => o._id === orderId);
    if (!order) {
    ...
    const shippingContainerEl = document.getElementById('modal-shipping-details-container');
    const shippingEl = document.getElementById('modal-shipping-details');
    const totalEl = document.getElementById('modal-order-total');

    // Poblar el modal con los datos del pedido
    orderIdEl.textContent = `Detalles del Pedido #${order.orderId || order._id}`;
    
    const { statusClass, statusText, statusIcon } = getOrderStatusInfo(order.status);
    statusEl.className = `status-badge-new ${statusClass}`;
    statusEl.innerHTML = `<i class="fas ${statusIcon}"></i> <span>${statusText}</span>`;

    itemsEl.innerHTML = order.items.map(item => `
        <div class="order-item-detail">
            <img src="${item.image || '/public/images/placeholder.png'}" alt="${item.name || 'Producto'}" onerror="this.onerror=null; this.src='/public/images/placeholder.png';">
            <div class="item-info-detail">
                <strong>${item.name || 'Producto Desconocido'}</strong>
                <span>${item.quantity || 0} x $${(parseFloat(item.price) || 0).toFixed(2)}</span>
            </div>
            <strong class="item-price-detail">$${((parseFloat(item.price) || 0) * (item.quantity || 0)).toFixed(2)}</strong>
        </div>
    `).join('');

    if (order.deliveryDetails) {
        shippingContainerEl.style.display = 'block';
        ...
        shippingContainerEl.style.display = 'none';
    }

    totalEl.textContent = `$${(parseFloat(order.totalAmount) || 0).toFixed(2)}`;
...
async function handleDeactivateAccountSubmit(event) {
    event.preventDefault();
    const form = event.target;
    ...
    try {
        const result = await makeApiCall('/api/users/deactivate', 'POST', { password });
        
        if (result && result.success) {
            showNotification("Tu cuenta ha sido eliminada exitosamente", "success");
            // Cerrar el modal
            const modal = document.querySelector(SELECTORS.DEACTIVATE_ACCOUNT_MODAL);
            if (modal) modal.style.display = 'none';
